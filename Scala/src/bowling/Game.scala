...
object Game {
  ...
  def roll (pins: List[Int]): Int = nextFramesTotal(1, pins)

  private def sumOfNextThreeRolls(pins: List[Int]): Int = pins.take(3).sum
  private def sumOfNextTwoRolls(pins: List[Int]): Int = pins.take(2).sum
  private def lastFrame(frame: Int): Boolean = frame == 10
  private def lastFrameTotal(pins: List[Int]): Int = pins.sum
  private def firstRollIsStrike(pins: List[Int]) : Boolean = pins.head == 10
  private def frameEndsWithSpare(pins: List[Int]) : Boolean = !firstRollIsStrike(pins) & sumOfNextTwoRolls(pins) == 10
